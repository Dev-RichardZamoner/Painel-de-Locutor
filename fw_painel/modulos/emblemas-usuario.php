...
<div id="base_text_topo_local_pagina" class="text_barra_title_pagina">Dire&ccedil;&atilde;o de conte&uacute;do / Emblemas usuarios</div>
<div id="content_title_pagina" class="title_pagina_barra"><span class="text_barra_title_pagina">Emblemas usuarios</span></div>
</div>
<script>
var apagar = {
	sim:function(id){
...
$imagem = $_POST['imagem'];
if($_POST){
	db::Query("INSERT INTO usuarios_emblemas(usuario, sobre, imagem) VALUES ('$habbo','$sobre','$imagem') ");
	echo Site::Alerta('Criado com sucesso!','index.php?cp='.$cp.'&c='.$c.'');
}
?>
<form method="post">
  	Usuario:<br>
    <input type="text" name="habbo"><br>
...
<?php
	...
	$imagem = $_POST['imagem'];
	if($_POST){
		db::Query("UPDATE usuarios_emblemas SET usuario='$habbo', sobre='$sobre', imagem='$imagem' WHERE id='$id'");
		echo Site::Alerta('Editado com sucesso!','index.php?cp='.$cp.'&c='.$c.'');
	}
	$sql = db::Query("SELECT * FROM usuarios_emblemas WHERE id='$id'");
	$ver = db::FetchArray($sql);
?>
<form method="post">
	
    Usuario:<br>
    <input type="text" name="habbo" value="<?php echo $ver['usuario']; ?>"><br>
    Descri&ccedil;&atilde;o:<br>
    <input type="text" name="sobre" value="<?php echo $ver['sobre']; ?>"><br>
    Imagem:<br>
...
<?php
	...
	$id = (int) $_GET['id'];
	db::Query("DELETE FROM usuarios_emblemas WHERE id='$id'");
	echo('deu');
}else{
	$pagina = $_GET['pagina'];
	if($pagina == 0){
	...
	$sql_total = "SELECT id FROM usuarios_emblemas";
...
<table width="100%">
    ...
    <?php
	$i = 1;
    $sql = db::Query("SELECT * FROM usuarios_emblemas ORDER BY id DESC LIMIT $inicio, $quantidade");
    ...
    <tr>
    	...
    	<th<?php echo $css; ?>><a href="index.php?cp=3&c=68&tipo=editar&id=<?php echo $ver['id']; ?>"><img src="imagens/edit.png"></a></th>
        <th<?php echo $css; ?>><?php echo $ver['id']; ?></th>
        <th<?php echo $css; ?>><?php echo $ver['usuario']; ?></th>
        <th<?php echo $css; ?>><?php echo $ver['sobre']; ?></th>
